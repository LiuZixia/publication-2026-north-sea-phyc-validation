# Screen DS11 BSH phytoplankton against the domain geometry.
...
pin <- utils::read.csv("metadata/stage2/acquisition/ds11_plet_chlorophyll_active_run.csv",
                       stringsAsFactors = FALSE, check.names = FALSE)
if (nrow(pin) != 1L || pin$work_item_id[[1]] != "REGISTER:DS11") {
  stop("DS11 active run pin is invalid.", call. = FALSE)
}
...
if (!identical(calculate_checksum(manifest_path), pin$manifest_checksum_sha256[[1]])) {
  stop("DS11 manifest checksum does not reconcile with the active pin.", call. = FALSE)
}

manifest <- utils::read.csv(manifest_path, stringsAsFactors = FALSE, check.names = FALSE)
csv_paths <- file.path(run_dir, manifest$file_name)
for (p in seq_along(csv_paths)) {
  if (!identical(calculate_checksum(csv_paths[[p]]), manifest$checksum_sha256[[p]])) {
    stop("DS11 raw CSV checksum does not match manifest.", call. = FALSE)
  }
}

raw_data <- do.call(rbind, lapply(csv_paths, function(p) {
...
if (length(missing_cols) > 0) {
  stop(sprintf("DS11 missing required columns: %s", paste(missing_cols, collapse = ", ")), call. = FALSE)
}
...
summary_path <- "metadata/stage2/screening/ds11_plet_chlorophyll_location_summary.csv"
write_csv_atomic(screening_summary, summary_path)

message(sprintf(
  "DS11 screening complete: %d core, %d external, %d outside locations out of %d total records.",
