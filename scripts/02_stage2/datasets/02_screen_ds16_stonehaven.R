# Screen DS16 BSH phytoplankton against the domain geometry.
...
pin <- utils::read.csv("metadata/stage2/acquisition/ds16_plet_stonehaven_active_run.csv",
                       stringsAsFactors = FALSE, check.names = FALSE)
if (nrow(pin) != 1L || pin$work_item_id[[1]] != "REGISTER:DS16") {
  stop("DS16 active run pin is invalid.", call. = FALSE)
}
...
if (!identical(calculate_checksum(manifest_path), pin$manifest_checksum_sha256[[1]])) {
  stop("DS16 manifest checksum does not reconcile with the active pin.", call. = FALSE)
}

manifest <- utils::read.csv(manifest_path, stringsAsFactors = FALSE, check.names = FALSE)
if (nrow(manifest) != 1L) stop("DS16 manifest should have exactly one file.", call. = FALSE)

csv_path <- file.path(run_dir, manifest$file_name[[1]])
if (!identical(calculate_checksum(csv_path), manifest$checksum_sha256[[1]])) {
  stop("DS16 raw CSV checksum does not match manifest.", call. = FALSE)
}
...
if (length(missing_cols) > 0) {
  stop(sprintf("DS16 missing required columns: %s", paste(missing_cols, collapse = ", ")), call. = FALSE)
}
...
summary_path <- "metadata/stage2/screening/ds16_plet_stonehaven_location_summary.csv"
write_csv_atomic(screening_summary, summary_path)

message(sprintf(
  "DS16 screening complete: %d core, %d external, %d outside locations out of %d total records.",
