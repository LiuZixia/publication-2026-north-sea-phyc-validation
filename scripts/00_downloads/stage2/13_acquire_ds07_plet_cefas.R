# Acquire canonical PLET export for DS07: Cefas SmartBuoy
# PLET dataset ID: "Cefas SmartBuoy Marine Observational Network - UK Waters Phytoplankton Data 2001-2019"
...
if (!work_item_id %in% work_order$work_item_id) {
  stop("DS07 is not in the Stage 2 work order.", call. = FALSE)
}
if (status$acquisition_state[status$work_item_id == work_item_id] == "complete") {
  stop("DS07 is already marked complete.", call. = FALSE)
}
...
dataset_name <- "UK Centre for Environment, Fisheries and Aquaculture Science (Cefas) - Cefas SmartBuoy Marine Observational Network - UK Waters Phytoplankton Data 2001-2019"
...
run_name <- paste0("DS07_PLET_CEFAS_", timestamp)
run_dir <- file.path("data", "raw", "stage2", "ds07_plet_cefas", run_name)
if (!dir.exists(run_dir)) dir.create(run_dir, recursive = TRUE)

html_path <- file.path(run_dir, "response.html")
csv_path <- file.path(run_dir, "cefas_smartbuoy_abundance.csv")
log_path <- file.path(run_dir, "acquisition.log")

cat(sprintf("Beginning acquisition of DS07 from PLET: %s\n", url), file = log_path)
...
manifest <- data.frame(
  file_name = "cefas_smartbuoy_abundance.csv",
...
pin <- data.frame(
  ...
  run_name = run_name,
  run_relative_path = file.path("stage2", "ds07_plet_cefas", run_name),
  manifest_checksum_sha256 = calculate_checksum(manifest_path),
  pinned_at_utc = timestamp,
  stringsAsFactors = FALSE,
  check.names = FALSE
)
write_csv_atomic(pin, "metadata/stage2/acquisition/ds07_plet_cefas_active_run.csv")

message("DS07 PLET acquisition complete.")
