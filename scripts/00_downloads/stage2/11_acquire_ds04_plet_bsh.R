# Acquire canonical PLET export for DS04: BSH phytoplankton
# PLET dataset ID: "BSH_Phyto_Zoo"
...
if (!work_item_id %in% work_order$work_item_id) {
  stop("DS04 is not in the Stage 2 work order.", call. = FALSE)
}
if (status$acquisition_state[status$work_item_id == work_item_id] == "complete") {
  stop("DS04 is already marked complete.", call. = FALSE)
}
...
run_name <- paste0("DS04_PLET_BSH_", timestamp)
run_dir <- file.path("data", "raw", "stage2", "ds04_plet_bsh", run_name)
...
cat(sprintf("Beginning acquisition of DS04 from PLET: %s\n", url), file = log_path)
...
manifest <- data.frame(
  file_name = "bsh_phyto_zoo_abundance.csv",
...
write_csv_atomic(pin, "metadata/stage2/acquisition/ds04_plet_bsh_active_run.csv")

message("DS04 PLET acquisition complete.")
