...
geometry_evidence_path <- stage2_relocated_path(cfg$geometry_evidence_path)
if (!identical(cfg$classification, "stage2_canonical_provider_and_licence_resolution") ||
    !identical(calculate_checksum(geometry_evidence_path), cfg$geometry_evidence_checksum_sha256)) {
  stop("Stage 2 WFS survivor metadata configuration or geometry evidence is invalid.", call. = FALSE)
}
evidence <- utils::read.csv(geometry_evidence_path, stringsAsFactors = FALSE,
...
summary <- list(
  ...
  geometry_evidence_checksum_sha256 = calculate_checksum(geometry_evidence_path),
