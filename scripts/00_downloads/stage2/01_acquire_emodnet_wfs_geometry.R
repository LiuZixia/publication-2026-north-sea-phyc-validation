...
queue_path <- stage2_relocated_path(cfg$queue_path)
...
if (!identical(calculate_checksum(queue_path), cfg$queue_checksum_sha256) ||
...
queue <- utils::read.csv(queue_path, stringsAsFactors = FALSE, check.names = FALSE,
...
run_summary <- list(
  ...
  queue_checksum_sha256 = calculate_checksum(queue_path),
