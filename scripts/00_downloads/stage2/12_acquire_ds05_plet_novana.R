# Acquire canonical PLET export for DS05: NOVANA phytoplankton
# PLET dataset ID: "NOVANA phytoplankton"
...
if (!work_item_id %in% work_order$work_item_id) {
  stop("DS05 is not in the Stage 2 work order.", call. = FALSE)
}
if (status$acquisition_state[status$work_item_id == work_item_id] == "complete") {
  stop("DS05 is already marked complete.", call. = FALSE)
}
...
dataset_name <- "NOVANA phytoplankton"
...
run_name <- paste0("DS05_PLET_NOVANA_", timestamp)
run_dir <- file.path("data", "raw", "stage2", "ds05_plet_novana", run_name)
if (!dir.exists(run_dir)) dir.create(run_dir, recursive = TRUE)

html_path <- file.path(run_dir, "response.html")
csv_path <- file.path(run_dir, "novana_phyto_abundance.csv")
log_path <- file.path(run_dir, "acquisition.log")

cat(sprintf("Beginning acquisition of DS05 from PLET: %s\n", url), file = log_path)
...
manifest <- data.frame(
  file_name = "novana_phyto_abundance.csv",
...
pin <- data.frame(
  ...
  run_name = run_name,
  run_relative_path = file.path("stage2", "ds05_plet_novana", run_name),
  manifest_checksum_sha256 = calculate_checksum(manifest_path),
  pinned_at_utc = timestamp,
  stringsAsFactors = FALSE,
  check.names = FALSE
)
write_csv_atomic(pin, "metadata/stage2/acquisition/ds05_plet_novana_active_run.csv")

message("DS05 PLET acquisition complete.")
