...
inventory_path <- stage2_relocated_path(cfg$inventory_path)
if (!identical(cfg$classification, "stage2_method_reference_acquisition") ||
    ...
    !identical(calculate_checksum(inventory_path), cfg$inventory_checksum_sha256) ||
...
write_tracked_pins <- function(directory, manifest) {
  ...
  write_csv_atomic(manifest, "metadata/stage2/acquisition/ds26_ifcb_reference_acquisition_manifest.csv")
  write_csv_atomic(pin, "metadata/stage2/acquisition/ds26_ifcb_reference_active_run.csv")
}
