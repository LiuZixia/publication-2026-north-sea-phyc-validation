...
active <- utils::read.csv("metadata/stage1_active_runs.csv", stringsAsFactors = FALSE, check.names = FALSE)
...
utils::write.csv(registry, "metadata/candidate_registry.csv", row.names = FALSE, na = "")
...
flow <- data.frame(
  ...
  generated_from = "metadata/candidate_registry.csv and pinned raw response parsers",
  generated_utc = stage1_now(), stringsAsFactors = FALSE
)
