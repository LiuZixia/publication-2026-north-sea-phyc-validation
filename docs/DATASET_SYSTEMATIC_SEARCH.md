...
### 13.4 Stage 2 record-level resolution of the 40 WFS candidates — 2026-08-08

The Stage 1 `pending` handoff above is preserved as the discovery-stage state. Stage 2 subsequently froze a title-neutral routing queue in [`metadata/stage2_wfs_geometry_queue.csv`](../metadata/stage2_wfs_geometry_queue.csv) and queried all 40 dataset IDs through the official dataset-keyed `Dataportal:eurobis-obisenv_full` WFS route. The complete finalized run is pinned in [`metadata/stage2_emodnet_wfs_active_run.csv`](../metadata/stage2_emodnet_wfs_active_run.csv): 81 raw responses cover 40 of 40 IDs and 384,548 records returned by the frozen-domain bounding box. Every response checksum was verified before exact local intersection with the unchanged Stage 0 polygon.

[`metadata/stage2_emodnet_wfs_geometry_evidence.csv`](../metadata/stage2_emodnet_wfs_geometry_evidence.csv) is the generated record-level result. Thirty-seven candidates have zero polygon-intersecting records. Three survive geometry screening:

- WFS 2453, national Swedish phytoplankton monitoring, retains 91,148 records in the Skagerrak/Kattegat external-transfer region;
- WFS 6698, regional Swedish phytoplankton monitoring, retains 280,399 records, of which 28,659 intersect the core-domain polygon and 251,740 the external-transfer polygon; and
- WFS 5951, `Dynamics of coastal eutrophicated ecosystems`, retains 71 records at one Belgian core-domain position.

Official MarineInfo metadata for the three survivors is archived and pinned separately. WFS 2453 and 6698 are CC0 aggregator copies of canonical SMHI/SHARK holdings, so they route to existing rank-1 `REGISTER:DS06`; they are neither new independent networks nor new work-order rows. WFS 5951 is CC-BY 4.0 and documents a targeted 1995–1996 weekly `Diatoma`/*Phaeocystis globosa* density programme. It is not total-community biomass and two years cannot meet the prespecified recurrence rule in `OUTCOME_AND_LIFEFORM_PROTOCOL.md` §7. It is therefore retained only as Tier F exploratory *Phaeocystis* discovery sensitivity. The generated dispositions and evidence links are in [`metadata/stage2_emodnet_wfs_screening.csv`](../metadata/stage2_emodnet_wfs_screening.csv) and [`metadata/stage2_emodnet_wfs_survivor_resolution.csv`](../metadata/stage2_emodnet_wfs_survivor_resolution.csv).

This result closes the direct-route uncertainty without changing the 19-row Stage 2 rank order. It also replaces the previously untested assumption of complete OBIS equivalence with a narrower demonstrated statement: the only confirmatory-grade in-domain WFS survivors found here belong to the already ranked SMHI provider family; the one additional core series is short, targeted, and exploratory only.

### 13.5 Rank-1 DS06 canonical-provider acquisition — 2026-08-08

The first ranked work item is now acquired from the canonical SMHI SHARK API rather than from the WFS aggregator copies. The checksum-pinned Stage 1 SHARK catalogue contains 219 versioned phytoplankton packages; dataset titles do not reliably encode record geography, so the prospective acquisition rule retrieved all 219 before applying the exact frozen polygon locally. The completed CC0 run is pinned by [`metadata/stage2_ds06_smhi_shark_active_run.csv`](../metadata/stage2_ds06_smhi_shark_active_run.csv) and its file-level provenance is in [`metadata/stage2_ds06_smhi_shark_acquisition_manifest.csv`](../metadata/stage2_ds06_smhi_shark_acquisition_manifest.csv). The immutable ZIP files total 27,700,519 bytes and every package passed CRC validation.

The R inventory reconciles 909,693 source rows across both SHARK export schemas. Exact local screening assigns 7,948 rows to the core domain, 478,860 to the prespecified Skagerrak/Kattegat external-transfer region, and 422,885 outside the frozen domain. The source contains 209,656 carbon-concentration rows, 186,334 biovolume-concentration rows, and 513,703 abundance or count rows; these are source-record counts, not sample totals or independent observations. The generated variable and package inventories are [`metadata/stage2_ds06_smhi_shark_variable_inventory.csv`](../metadata/stage2_ds06_smhi_shark_variable_inventory.csv) and [`metadata/stage2_ds06_smhi_shark_package_summary.csv`](../metadata/stage2_ds06_smhi_shark_package_summary.csv).

Provider sample identity found 8,102 unique samples and only four sample IDs shared across packages. Those eight sample-package memberships were re-read from the immutable ZIPs and compared using SHA-256 fingerprints over 51 sample, taxon, size-class, measurement, method, and reported-value fields. No cross-package rows were identical under that rule, so the audit preserves all of them rather than collapsing similar measurements. The evidence is in [`metadata/stage2_ds06_smhi_shark_sample_overlap.csv`](../metadata/stage2_ds06_smhi_shark_sample_overlap.csv) and [`metadata/stage2_ds06_smhi_shark_duplicate_resolution_summary.csv`](../metadata/stage2_ds06_smhi_shark_duplicate_resolution_summary.csv).

DS06 is provisionally Tier A because the canonical files supply taxon-level carbon concentration directly, but it is not finally eligible. [`metadata/stage2_ds06_smhi_shark_screening_summary.csv`](../metadata/stage2_ds06_smhi_shark_screening_summary.csv) retains a `pending` decision until the exact CMEMS product temporal coverage is frozen and Stage 5 establishes compatible sample-level total-community construction and method epochs. No PhyC file or value was used in acquisition, spatial screening, duplicate assessment, or this provisional tier assignment.

### 13.6 Rank-2 DS26 SMHI IFCB acquisition — 2026-08-08

The Stage 1 entry had conflated two related but scientifically different resources: the recurrent SMHI-published IFCB observation stream and the manually annotated image reference library used to train or audit its classifiers. Stage 2 now preserves that distinction. The provider-managed [GBIF Sweden IPT resource](https://www.gbif.se/ipt/resource?r=shark-planktonimaging) is the observation source; its EML and 18.3 MB Darwin Core archive are pinned by [`metadata/stage2_ds26_smhi_ifcb_active_run.csv`](../metadata/stage2_ds26_smhi_ifcb_active_run.csv). The separate SciLifeLab Figshare v6 item is method evidence, not an independent monitoring network.

The recurrent archive contains 17,731 event-table rows, 121,103 machine-classified occurrence rows, and 1,111,062 extended measurement/fact rows. The event table is a three-level Darwin Core hierarchy: one `dataset_event` container, 8,865 `sampling_event` parents, and 8,865 `sample` leaves. Occurrences attach to 8,864 sample leaves. Thus the raw count of 8,867 occurrence-unlinked event-table rows comprises 8,865 structural parents, one dataset container, and one unlinked leaf sample — not 8,867 zero-detection samples. All 17,731 rows have event-level measurements. The one unlinked leaf has an imaging-instrument flag of `Missing cells` and an unidentified ROI; its ecological meaning remains unresolved and it is `unknown`, never negative, pending provider/QC qualification. This generated audit is pinned in [`metadata/stage2_ds26_smhi_ifcb_event_linkage_audit.csv`](../metadata/stage2_ds26_smhi_ifcb_event_linkage_audit.csv).

The archive reports occurrence-linked abundance, biovolume concentration, carbon content, classifier identity, classifier F1 score, and trophic type, plus event-level unidentified-ROI abundance/volume and instrument metadata. Exact frozen-geometry screening retains 3,212 core-domain occurrences and 57,844 in the external-transfer region; 60,047 Baltic or other rows are outside the domain and no occurrence lacks a valid coordinate. Generated evidence is in [`metadata/stage2_ds26_smhi_ifcb_event_summary.csv`](../metadata/stage2_ds26_smhi_ifcb_event_summary.csv), [`metadata/stage2_ds26_smhi_ifcb_measurement_summary.csv`](../metadata/stage2_ds26_smhi_ifcb_measurement_summary.csv), and [`metadata/stage2_ds26_smhi_ifcb_screening_summary.csv`](../metadata/stage2_ds26_smhi_ifcb_screening_summary.csv).

The three provider series are Tångesund in 2016, Baltic records in 2022–2024, and Skagerrak/Kattegat records in 2022–2024. Thus only four calendar years are represented, despite high within-year frequency. Classifications are explicitly `PredictedByMachine`, and the imaged size domain cannot be assumed to represent total phytoplankton including pico/nanoplankton. DS26 is therefore retained as a provisional Tier B `lifeform_only` secondary temporal-resolution benchmark; it is not recurrent total-community truth and does not add a second transition-region network independent of DS06.

The full [SMHI IFCB Plankton Image Reference Library v6](https://doi.org/10.17044/scilifelab.25883455.v6) is archived separately under CC BY 4.0: four files, 8,104,707,049 bytes, all source-size/MD5/SHA-256 verified and both ZIPs CRC-tested. Its provider README documents 86,232 expert-annotated images across 146 classes from 2016–2026. [`metadata/stage2_ds26_ifcb_reference_summary.csv`](../metadata/stage2_ds26_ifcb_reference_summary.csv) assigns it solely to classifier, taxonomy, size-range, and method evidence. Image rows never enter the observation count or duplicate-map network count.

No CMEMS file or PhyC value informed either DS26 disposition. Exact CMEMS temporal overlap remains unassessed pending the product-metadata freeze; this does not alter the observation-only finding that four years cannot satisfy the registered recurrent-lifeform criterion.

### 13.7 Rank-3 DS02 canonical RWS route — 2026-08-09

DS02 was started from Rijkswaterstaat rather than from the public PLET extract. The official RWS phytoplankton page now routes biological monitoring through Waterinfo and WaterWebservices. `scripts/00_downloads/08_acquire_ds02_rws_catalogue.R` archived that page, the official CC0 WaterWebservices documentation, and a complete extended DD API 2.0 catalogue response with biological/taxon fields. The pinned response contains 2,757 measurement definitions, 2,635 locations, and 104,679 metadata–location links.

Exact screening against the frozen geometry assigns 1,092 catalogue locations to the core, none to the external-transfer regions, 1,543 outside the domain, and none to invalid coordinates. Of the measurement definitions, 2,158 link to at least one in-domain location and 21 carry a count-per-volume or biovolume signal, but none explicitly identifies marine phytoplankton under the frozen search terms. This is a route diagnosis, not evidence that phytoplankton observations or blooms are absent. The generated evidence is pinned in `metadata/stage2_ds02_rws_catalogue_link_summary.csv` and `metadata/stage2_ds02_rws_catalogue_output_registry.csv`.

The current RWS biological DD API V3 endpoint was then queried without authentication by `scripts/00_downloads/09_diagnose_ds02_rws_v3_access.R`. Its archived response is HTTP 401 with a zero-byte body; that API diagnosis acquired no observation rows. PLET remains duplicate/completeness evidence and was not substituted for canonical observations. The catalogue and access-diagnosis zero counts mean not acquired by those routes, never ecological absence or a negative window.

On 2026-08-09 the user obtained a canonical provider export through the [Waterinfo bulk-download portal](https://waterinfo.rws.nl/nav/bulkdownload), selecting `Overige biologische data` and `Taxon type` values `FYTPT` or `DIATM`. The portal delivered four CSV parts. `scripts/00_downloads/10_intake_ds02_rws_manual_export.R` preserves and verifies those files byte-for-byte, records why the manual route was needed, and pins their checksums in `metadata/stage2_ds02_rws_manual_export_acquisition_manifest.csv`. No machine-readable portal receipt was delivered and other portal filters are therefore recorded as `not_reported`, not inferred.

The four parts total 76,933,080 bytes and 505,452 observation rows across 139 meetobjects, with no station overlap between parts. Their observed dates span 2000-01-09 through 2025-09-02: 432,532 rows fall in the registered DS02 period 2000–2019 and 72,920 lie outside it. The broader raw delivery is retained unchanged; the later record screen, not the intake, will apply the temporal and exact spatial boundaries. The files contain both `AANTPVLME` abundance and `AANTFTE` fraction records, units `N/L`, `N/ML`, and `PROCENT`, and compartments `OW`, `HY`, `MN`, and `KS`.

Two provider-export issues are registered prospectively for screening. There are 1,574 byte-identical data rows, which remain unresolved duplicate candidates. Part 4 also contains 105 rows where Waterinfo inserted an unheaded measurement-object description after `Meetobject`; the script verifies the exact anomaly signature and aligns it only in memory for inventory calculations without changing the raw file. DS02 remains provisional Tier C, `primary_reference`, and `pending` until all 505,452 rows are joined to provider coordinates, restricted to the frozen period/domain, duplicate-screened, and audited for method epochs and biomass convertibility. No exported row is yet a sample, event, negative window, or eligible reference.
