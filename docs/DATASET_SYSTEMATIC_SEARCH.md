...
The current RWS biological DD API V3 endpoint was then queried without authentication by `scripts/00_downloads/09_diagnose_ds02_rws_v3_access.R`. Its archived response is HTTP 401 with a zero-byte body; no observation rows were acquired. DS02 therefore remains provisional Tier C, `primary_reference`, and `pending` while provider credentials or a canonical Waterinfo export are obtained. PLET remains duplicate/completeness evidence and is not substituted for the canonical observations. All zero record counts mean not yet acquired, never ecological absence or a negative window.
