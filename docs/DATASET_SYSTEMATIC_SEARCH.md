...
- **Machine-readable log:** [`metadata/stage1/input/manual_discovery_log.csv`](../metadata/stage1/input/manual_discovery_log.csv)
...
All remaining URLs cited in §12.2–§12.4 are `search_summary_only` and are listed with their status in [`metadata/stage1/input/manual_discovery_log.csv`](../metadata/stage1/input/manual_discovery_log.csv).

## 13. Executed Stage 1 and Stage 2 handoff — 2026-08-08 onward

Stage 1 was executed reproducibly against its frozen configuration, followed by a separately pinned append-only EMODnet Biology WFS audit. Its single operational description is [STAGE1.md](stages/STAGE1.md). The authoritative generated evidence is under [metadata/stage1/](../metadata/stage1/): raw-run pins, query provenance, candidate registry, calculated flow, register crosswalk, ranked Stage 2 handoff, and complete file inventory.

The requirement audit repaired biological-scope, DOI-missingness, provider-code, title-normalization, collection-DOI, geographic-state, character-encoding, and known-item-recall defects before the handoff was accepted. Dataset-level geography remains recorded rather than enforced. Offshore evidence remains low tier, direct-carbon evidence is limited, and final eligibility still depends on later record-, method-, recurrence-, and biomass-conversion audits. Use the generated registries for all counts and exact dispositions; the narrative candidate tables above are search protocol and discovery context, not calculated results.

Stage 2 preserved that discovery state, froze the ranked work order, and performed record-level acquisition and screening without inspecting CMEMS PhyC. Its single operational description is [STAGE2.md](stages/STAGE2.md). The generated current state is [acquisition_status.csv](../metadata/stage2/control/acquisition_status.csv), and complete file provenance is in [file_inventory.csv](../metadata/stage2/inventory/file_inventory.csv).

The executed scientific dispositions remain:

- EMODnet WFS candidates were resolved using archived record geometry and official metadata; aggregator copies route to their canonical provider family, while short targeted series remain exploratory rather than recurrent total-biomass truth.
- DS06 canonical SMHI/SHARK observations remain a provisional direct-carbon primary reference pending later sample compatibility, method-epoch, and exact CMEMS-overlap audits.
- DS26 recurrent IFCB observations remain secondary lifeform/method evidence rather than recurrent total-community truth; the separate annotated image library is classifier-method evidence and never an independent monitoring network.
- DS02 uses canonical RWS Waterinfo files. Catalogue or access-route zero counts are route diagnoses, never ecological absences; record-level eligibility and biomass construction remain pending.
- DS12 public CPR events are retained only as Tier D/E PCI and taxon recurrence evidence, never total-community biomass. Its provider email is an optional metadata enhancement, not an access blocker.
- DS27 FerryBox chlorophyll fluorescence is retained only as a Tier E comparator; the separately archived pCO2-only component is excluded from phytoplankton evidence.
- Contact-required datasets are temporarily unavailable under the frozen access policy. The generated access register records why email is necessary, the consequence of proceeding without each dataset, and the evidence required to revisit the disposition. Missing access is never converted to an ecological zero.
- Acquired datasets can be complete for Stage 2 while retaining a `pending` scientific inclusion decision owned by later observation-only coverage, method, overlap, and biomass audits.

All quantitative acquisition, geometry, duplicate, variable, event, and screening results are generated under [metadata/stage2/](../metadata/stage2/). Use those files and their producer scripts for publication values. Acquisition code under [scripts/00_downloads/stage2/](../scripts/00_downloads/stage2/) retains the exact provider requests and is invoked separately from the offline Stage 2 validation gate.
