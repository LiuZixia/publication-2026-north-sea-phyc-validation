...
- **Updated (UTC):** 2026-08-08T16:55:00Z
- **Current stage:** Stage 1 complete with a direct EMODnet Biology WFS append audit. Stage 2 has not started. Do not begin CMEMS acquisition or PhyC inspection.
...
**Commit the fully validated EMODnet closure separately from checkpoint `df7f98f`.**

Completion evidence already obtained: the registry, crosswalk, shortlist, query log, and WFS overlap artefacts reproduced byte-for-byte and all Stage 0/1 tests passed in `outputs/logs/stage1_validation_20260808T165447Z.log`. Remaining evidence is the separate commit on top of `df7f98f`.

## Ordered Next Actions

1. Regenerate `outputs/stage_status.md`, stage the reconciled append files, and commit the EMODnet closure separately from `df7f98f`.
2. Freeze the Stage 2 record-level screening contract. It must explicitly resolve the 40 unmatched biological-title WFS candidates: 18 title-diagnosed out of domain and 22 with unknown dataset-level geography.
3. Acquire in shortlist rank order beginning DS06, DS26, DS02, DS04, DS05, and DS07.
