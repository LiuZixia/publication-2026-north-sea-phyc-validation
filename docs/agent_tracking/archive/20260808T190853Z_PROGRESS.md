...
- **Current stage:** Stage 2 contract frozen and the 40-candidate EMODnet WFS record-level gate completed; ranked canonical-provider acquisition has not yet begun and CMEMS PhyC remains untouched.
- **Session objective:** Completed: freeze and test the Stage 2 record-level screening contract and resolve all 40 unmatched WFS candidates before ranked observation acquisition.
- **Branch/commit:** `main` starting at clean commit `648d948` (`stage1: audit EMODnet Biology WFS holdings`).
- **First concrete next action:** Commit this validated Stage 2 contract/WFS checkpoint, then acquire the canonical SHARK/SMHI packages routed through `REGISTER:DS06` in rank order.
- **Last completed milestone:** Stage 2 contract and EMODnet record-level closure validated at 2026-08-08T19:08:26Z.
- **Archive snapshot:** `docs/agent_tracking/archive/20260808T181024Z_PROGRESS.md` and `..._PENDING.md` preserve the Stage 1 handoff at this session start.
...
### Stage 2 contract and EMODnet record-level closure

- `config/stage2_record_screening_contract.json` prospectively freezes immutable-file provenance, variable inventory, stable source-record identity, record-level eligibility, licence state, duplicate linkage, and candidate/work-order schemas before ranked observation acquisition.
- `metadata/stage2_acquisition_work_order.csv` preserves all 19 Stage 1 shortlist entries in exact rank order. `metadata/stage2_contract_freeze.json` pins the contract, shortlist, spatial inputs, access policy, and generated routing artifacts by SHA-256; repeated initialization is byte-identical.
- `metadata/stage2_wfs_geometry_queue.csv` retains all 40 unmatched WFS candidates as pending at the title-only handoff: 18 title-level out-of-domain signals and 22 unknowns. No title-only signal was used as a final decision.
- The official dataset-keyed `eurobis-obisenv_full` WFS route was archived in `data/raw/stage2/emodnet_wfs_geometry/20260808T183226Z`: 40/40 IDs queried, 81 checksum-registered responses, 384,548 bounding-box records, approximately 994 MB. An unbounded generic-layer preflight and a combined generic CQL/BBOX preflight were non-responsive or rejected; neither produced a finalized raw run.
- Exact intersection with the unchanged, checksum-pinned Greater North Sea polygon resolved all 40 candidates: 37 have zero intersecting records; 2453 retains 91,148 external-transfer records, 5951 retains 71 core records, and 6698 retains 280,399 records (28,659 core; 251,740 external-transfer). No returned point touches the polygon boundary.
- Official MarineInfo metadata for the three survivors is archived under `data/raw/stage2/emodnet_wfs_survivor_metadata/20260808T190338Z`. WFS 2453 and 6698 are open CC0 aggregator copies of canonical SMHI/SHARK holdings and route to ranked `REGISTER:DS06`, not new independent networks or new work-order rows.
- WFS 5951 is open CC-BY 4.0, but its official record is a targeted weekly 1995–1996 `Diatoma`/`Phaeocystis globosa` density series rather than total-community biomass. Two years cannot meet the prespecified recurrence rule, so it is retained only as Tier F exploratory *Phaeocystis* discovery sensitivity.
...
| `config/stage2_record_screening_contract.json`, `R/03_stage2_contract.R` | added | Prospectively freeze and validate Stage 2 schemas and scientific invariants | executed and tested |
| `scripts/02_initialize_stage2_contract.R`, `metadata/stage2_*work_order.csv`, `metadata/stage2_contract_freeze.json`, `metadata/stage2_wfs_geometry_queue.csv` | added/generated | Deterministic 19-row work order and 40-row title-neutral WFS queue | byte-stable |
| `config/stage2_emodnet_wfs_geometry.json`, `scripts/00_downloads/03_screen_emodnet_wfs_candidates.R` | added | Archive dataset-keyed WFS domain geometry evidence with pagination, timeouts, manifests, and checksums | complete |
| `scripts/02_register_stage2_wfs_run.R`, `scripts/02_screen_stage2_wfs_geometry.R` | added | Verify the raw run and apply exact frozen-polygon/subregion screening | executed |
| `metadata/stage2_emodnet_wfs_active_run.csv`, `metadata/stage2_emodnet_wfs_geometry_evidence.csv`, `metadata/stage2_emodnet_wfs_screening.csv` | added/generated | Pin the 81-response run and record all 40 geometry decisions | complete |
| `config/stage2_emodnet_wfs_survivor_metadata.json`, `scripts/00_downloads/04_download_emodnet_wfs_survivor_metadata.R` | added | Archive official provider, method, and licence evidence for the three geometry survivors | complete |
| `scripts/02_register_stage2_wfs_metadata_run.R`, `scripts/02_resolve_stage2_wfs_survivors.R`, `metadata/stage2_emodnet_wfs_survivor_resolution.csv` | added/generated | Route two aggregator copies to DS06 and one short targeted series to exploratory sensitivity | complete |
| `tests/test_stage2_contract.R`, `scripts/02_validate_stage2_contract.R` | added | Contract, determinism, geometry, licence, duplicate-routing, and PhyC-boundary regression coverage | passed |
...
- `Rscript scripts/00_downloads/03_screen_emodnet_wfs_candidates.R`: passed; finalized 40-dataset, 81-response run with 384,548 bounding-box records at `stage2/emodnet_wfs_geometry/20260808T183226Z`.
...
- `Rscript scripts/02_screen_stage2_wfs_geometry.R`: passed with the unchanged EPSG:4326 polygon evaluated as prepared GeoJSON linear rings; 37 excluded, three retained for provider resolution, zero boundary-touch records.
- `Rscript scripts/00_downloads/04_download_emodnet_wfs_survivor_metadata.R`: passed; three official MarineInfo pages archived and title-validated.
- `Rscript scripts/02_resolve_stage2_wfs_survivors.R`: passed; two open CC0 SMHI copies route to DS06 and one CC-BY Tier F series remains exploratory.
- `Rscript -e 'testthat::test_dir("tests", reporter="summary", stop_on_failure=TRUE)'`: all Stage 0–2 tests passed.
- `Rscript scripts/02_validate_stage2_contract.R`: passed; contract artifacts reproduced byte-for-byte; log `outputs/logs/stage2_contract_validation_20260808T190826Z.log`.
...
- The ranked observation work order remains 19 rows. The WFS audit adds no independent work-order row: two survivors route to DS06 and one is exploratory only.
