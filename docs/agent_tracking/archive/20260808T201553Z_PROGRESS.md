...
- **Current stage:** Stage 2 ranked acquisition in progress; rank-1 DS06 is committed and rank-2 DS26 canonical-route resolution is underway.
- **Session objective:** Acquire and record-screen the recurrent SMHI IFCB observation archive while keeping the separate annotated-image reference library in its proper method-evidence role.
- **Branch/starting commit:** `main` at `796b7c2` (`stage2: acquire and screen canonical DS06 SHARK data`).
- **First concrete next action:** Archive the SMHI-published IFCB Darwin Core archive and EML plus the official Figshare article metadata through a version-controlled acquisition module.
